...
export const showDealsController = async (req, res) => {
  ...
  async function showCarfn() {
    try {
      ...
      const showAllCars = database.collection("deals");
...
export const addDealsController = async (req, res) => {
    ...
    async function addCarfn() {
      try {
        const database = client.db("nscarwebsite");
        const carAccount = database.collection("deals");
